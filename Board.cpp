...
Board::Board(uint8_t size) {
    ...
    for (int i = 0; i < board_size + 2; i++) { //init board
...
int Board::text_to_move(std::string move) const {
    ...
    if (move[0] > 'i') { //i isnt allowed cause its too easy to confuse with 1 or something
        --column;
    }
...
void Board::set_state(uint16_t vertex, vertex_t new_state) {
    ...
    if (get_neighbors(vertex, new_state) == 0) { //no neighbors, new chain
        Chain x;
    ...
    } else if (get_neighbors(vertex, new_state) == 1) { //1 neighbor, add to chain
        for (int i = 0; i < 4; i++) { 
            int neighbor = vertex + directions[i];
            if (valid_vertex(neighbor)) {
                if (get_state(neighbor) == new_state) {
                    if (chain_reps[neighbor] != 255) {  //if not null chain
    ...
    } else {   //multiple neighbors, merge chains
        for (int i = 0; i < 4; i++) {
            int neighbor = vertex + directions[i];
            if (valid_vertex(neighbor)) {
                if (get_state(neighbor) == new_state) {
...
void Board::merge(uint16_t chain1, uint16_t chain2) {
    ...
    if (chain_reps[chain1] == 255 || chain_reps[chain2] == 255) {
        return; //TODO: figure out what 255 means here, some kind of invalid value
    }
    Chain current = chains[chain_reps[chain1]];
    Chain other = chains[chain_reps[chain2]];
    if (current.side != other.side) {
        return; //cant merge two chains of different colors
    }
    Chain new_chain;  //strategy: make new chain and add everything to it
    ...
    //this might be bad
    chains[chain_reps[chain1]].num_liberties = 0;
    chains[chain_reps[chain2]].num_liberties = 0;
    delete_chain(chain_reps[chain1]); //causing error cause don't have num_liberties = 0
...
void Board::capture_chain(uint16_t vertex) {
    ...
    assert(get_chain_liberties(vertex) == 0);
...
void Board::delete_chain(uint8_t chain_index) {
    ...
    assert(chain_index < chains.size());
    assert(chains[chain_index].num_liberties == 0);
